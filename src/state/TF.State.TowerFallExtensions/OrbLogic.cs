...
namespace TF.State.TowerFallExtensions
{
    ...
    {
        public static TF.State.Domain.Models.OrbLogic.OrbLogic GetState(this TowerFall.OrbLogic self)
        {
            var orb = new TF.State.Domain.Models.OrbLogic.OrbLogic();
        ...
        public static void LoadState(this TowerFall.OrbLogic self, TF.State.Domain.Models.OrbLogic.OrbLogic orb)
        {
            var dynOrb = DynamicData.For(self);
            ...
            dynTFGame.Set("DeltaTime", (float)Engine.Instance.TargetElapsedTime.TotalSeconds * orb.Time.EngineTimeRate);

            if (!orb.Time.GameRateEased)
            {
                ScreenEffects.Reset();
            }
