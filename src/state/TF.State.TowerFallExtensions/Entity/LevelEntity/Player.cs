...
using TF.State.TowerFallExtensions.CompositeComponent;
...
namespace TF.State.TowerFallExtensions.Entity.LevelEntity
{
    public static class PlayerExtensions
    {
        public static Player GetState(this TowerFall.Player entity)
        {
            ...
            var schedulerState = new TF.State.Domain.Models.Entity.LevelEntity.Player.Scheduler
            {
            ...
            return new TF.State.Domain.Models.Entity.LevelEntity.Player.Player
            {
                ...
                SpamShotCounter = dynPlayer.Get<Counter>("spamShotCounter").Value,
        ...
        private static TF.State.Domain.Models.Entity.LevelEntity.Player.Hitbox GetHitbox(TowerFall.Player self)
        {
            var dynPlayer = DynamicData.For(self);
            var normalHitbox = dynPlayer.Get<TowerFall.WrapHitbox>("normalHitbox");
            if (self.Collider == normalHitbox)
            {
                return TF.State.Domain.Models.Entity.LevelEntity.Player.Hitbox.Normal;
            }

            return TF.State.Domain.Models.Entity.LevelEntity.Player.Hitbox.Ducking;
        }

        private static void LoadHitbox(TowerFall.Player self, TF.State.Domain.Models.Entity.LevelEntity.Player.Hitbox hitbox)
        {
            var dynPlayer = DynamicData.For(self);

            switch (hitbox)
            {
                case TF.State.Domain.Models.Entity.LevelEntity.Player.Hitbox.Normal:
                    dynPlayer.Invoke("UseNormalHitbox");
                    break;
                case TF.State.Domain.Models.Entity.LevelEntity.Player.Hitbox.Ducking:
