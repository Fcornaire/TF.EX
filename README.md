...
| `AUTO ADJUST INPUT DELAY` | `PROPOSE`  | Adapts the input delay to the connection when joining a lobby: `PROPOSE` shows a suggestion in ms (half the laggiest player's ping, rounded up) you can accept or ignore, `ENABLED` applies it automatically. Your saved value is never changed |
